...
* comment text - placeholder '{user}' is replaced with actual assignee. Let them be ashamed!
* poke user - an user who posts notification. Admin by default.
* excluded projects - a comma-separated list of projects excluded from checking.
...
You can place into crontab something like that:
	...
	0 1 * * * cd REDMINE_ROOT && RAILS_ENV=production bundle exec rake issuepoke:poke
